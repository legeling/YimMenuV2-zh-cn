...
namespace YimMenu
{
	void DrawSavedVariableEdit(const SavedVariableBase& var, void* value)
	{
		...
		case VariableType::INT:
		{
			...
			ImGui::InputScalar(Localization::Translate("Value").c_str(), ImGuiDataType_S32, static_cast<int*>(value));
			break;
		}
		case VariableType::INT64:
		{
			ImGui::SetNextItemWidth(200.f);
			ImGui::InputScalar(Localization::Translate("Value").c_str(), ImGuiDataType_S64, static_cast<std::int64_t*>(value));
			break;
		}
		case VariableType::BITSET:
		{
			ImGui::SetNextItemWidth(200.f);
			ImGui::Bitfield(Localization::Translate("Value").c_str(), static_cast<int*>(value));
			break;
		}
		case VariableType::BOOL:
		{
			ImGui::Checkbox(Localization::Translate("Value").c_str(), static_cast<bool*>(value));
			break;
		}
		case VariableType::FLOAT:
		{
			ImGui::SetNextItemWidth(200.f);
			ImGui::InputScalar(Localization::Translate("Value").c_str(), ImGuiDataType_Float, static_cast<float*>(value));
			break;
		}
		...
		{
			ImGui::SetNextItemWidth(200.f);
			ImGui::InputText(Localization::Translate("Value").c_str(), static_cast<char*>(value), 255);
			break;
		}
		}
	}
	...
	void DrawSavedVariablePreview(const SavedVariableBase& var, void* value)
	{
		if (!value)
		{
			ImGui::Selectable(Localization::Translate("Invalid").c_str(), false, ImGuiSelectableFlags_Disabled);
			return;
		}
		...
		case VariableType::BOOL:
		{
			ImGui::Selectable(*static_cast<bool*>(value) ? Localization::Translate("True").c_str() : Localization::Translate("False").c_str(), false, ImGuiSelectableFlags_Disabled);
			break;
		}
		case VariableType::FLOAT:
		{
	...
	void DrawSavedVariable(SavedVariableBase& var)
	{
		...
		if (ImGui::Button(Localization::Translate("Add Offset").c_str()))
			var.appendages.push_back({VariableAppendage::Type::OFFSET, 0LL, 0ULL});
		ImGui::SameLine();
		if (ImGui::Button(Localization::Translate("Add Read Player Id").c_str()))
			var.appendages.push_back({VariableAppendage::Type::PLAYER_ID, 0LL, 0ULL});

		if (var.appendages.size() > 0)
		{
			ImGui::SameLine();
			if (ImGui::Button(Localization::Translate("Remove Offset").c_str()))
			{
